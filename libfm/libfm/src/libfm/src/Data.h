...
class Data {
	...
	public:	
		...
		void load(std::string filename);
                void load_withoutY(std::string filename);
		void debug();
			...
			if (sscanf(pline, "%f%n", &_value, &nchar) >=1) {
				...
				while (sscanf(pline, "%d:%f%n", &_feature, &_value, &nchar) >= 2) {
					...
					num_values++;	
                                ...
                                //printf("%d:%f\n",_feature, _value);
                                //printf("%s %c %d\n", pline, *pline, *pline);
				while ((*pline != 0) && ((*pline == ' ')  || (*pline == 9))) { printf("%c %d\n", *pline, *pline); pline++; } // skip trailing spaces
				if ((*pline != 13) && (*pline != 0)  && (*pline != '#')) { 
...
void Data::load_withoutY(std::string filename) {
	...
	// (1) determine the number of rows and the maximum feature_id
	{
		...
		while (!fData.eof()) {
			...
			//if (sscanf(pline, "%f%n", &_value, &nchar) >=1) 
                        {
				//pline += nchar;
				min_target = 0;//std::min(_value, min_target);
				max_target = 1;//std::max(_value, max_target);			
				num_rows++;
				while (sscanf(pline, "%d:%f%n", &_feature, &_value, &nchar) >= 2) {
					pline += nchar;	
					num_feature = std::max(_feature, num_feature);
