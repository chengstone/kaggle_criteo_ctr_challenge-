...
int main(int argc, char **argv) { 
	...
	try {
		...
		const std::string param_train_off 	= cmdline.registerParameter("train_off", "predict results without train");
		const std::string param_prefix		= cmdline.registerParameter("prefix", "prefix filename for logits output");
		const std::string param_test2predict	= cmdline.registerParameter("test2predict", "test file for predict into logits output");
		...
		Data train(
			...
			! (!cmdline.getValue(param_method).compare("sgd") || !cmdline.getValue(param_method).compare("sgda")) // no transpose data for sgd, sgda
                ...
                if(cmdline.hasParameter(param_test2predict) 
                && ! cmdline.getValue(param_test2predict).compare("true")){
                    train.load_withoutY(cmdline.getValue(param_train_file));
                } else {
		...
		// std::cout << cmdline.getValue(param_train_off)
		// 		<< cmdline.getValue(param_train_off).compare("false") << std::endl;	

		if (! cmdline.hasParameter(param_train_off) 
		|| ! cmdline.getValue(param_train_off).compare("false")) {
			// () learn		
		...
		// () Prediction at the end  (not for mcmc and als)
                if(cmdline.hasParameter(param_test2predict) 
                && ! cmdline.getValue(param_test2predict).compare("true")){
                    std::cout << "Final\t" << "Test=" << fml->evaluate(test) << std::endl;	
                } else 
		if (cmdline.getValue(param_method).compare("mcmc")) {
		...
		// std::cout << cmdline.hasParameter(param_out)
		// 		<< cmdline.hasParameter(param_train_off)
		// 		<< cmdline.getValue(param_train_off) 
		// 		<< cmdline.getValue(param_train_off).compare("true") << std::endl;	

		if (cmdline.hasParameter(param_train_off) 
		&& ! cmdline.getValue(param_train_off).compare("true")) {
			// std::cout << "--------train off--------start!" << std::endl;	
			DVector<double> pred;
			pred.setSize(train.num_cases);
			((fm_learn_sgd_element*)fml)->sgd_logits(train, pred);

			if (cmdline.hasParameter(param_prefix)){
				pred.save(cmdline.getValue(param_prefix) + ".fm.logits");	
			} else {
				pred.save("fm.logits");	
			}
			// std::cout << "--------train off--------end!" << std::endl;	
		}
